...
$sql_get_acc_income = "SELECT acc_gl_id,income_id FROM ACC_INCOME ";
$query_get_acc_income = $db->query($sql_get_acc_income);
while ($rec_get_acc_income = $db->db_fetch_array($query_get_acc_income)) {
	$arr_income_acc[$rec_get_acc_income["income_id"]] = $rec_get_acc_income["acc_gl_id"];
}
...
switch ($proc) {
	case "add": {
			try {
				...
				$sql_update =  "update  m_member  set deposit_money='" . $_POST['txt_after_money'] . "' where  member_id = '" . $member_id . "'";
				$db->query($sql_update);
				...
				if ($_POST['income_var'][$Arr_income['005']] > 0) {
					...
					$sql_arrear = "select * from  M_ARREAR where MEMBER_ID ='" . $member_id . "' and  FLAG_PAID = 0
						ORDER BY MEMBER_OUT_ID asc ";
				...
				//ในสมุดมียังไม่
				if (trim($dead_start) == '') {
					$sql_dead_start = "select min(dead_number_all) dead_start
										from  M_MEMBER_OUT
										join  M_ARREAR on M_MEMBER_OUT.member_out_id  = M_ARREAR.MEMBER_OUT_ID
										where  M_ARREAR.MEMBER_ID = '" . $member_id . "'
										AND  M_ARREAR.FLAG_PAID =1
											";
					$dead_start = $db->get_data_field($sql_dead_start, "dead_start");
				}

				$sql_dead_end = "select max(dead_number_all) dead_end
										from  M_MEMBER_OUT
										join  M_ARREAR on M_MEMBER_OUT.member_out_id  = M_ARREAR.MEMBER_OUT_ID
										where  M_ARREAR.MEMBER_ID = '" . $member_id . "'
										AND  M_ARREAR.FLAG_PAID =1
				...
				if (trim($dead_start)) {
					$sql_dead_end = "select sum(M_ARREAR.ARREAR_MONEY) dead_money
										from  M_MEMBER_OUT
										join  M_ARREAR on M_MEMBER_OUT.member_out_id  = M_ARREAR.MEMBER_OUT_ID
										where  M_ARREAR.MEMBER_ID = '" . $member_id . "'
				...
				//post บัญชี บันทึก
				$tablea_acc = "ACC_TRANSEC";
				$table_detail = "ACC_TRANSEC_DETAIl";
				unset($fields_detail);
				unset($fields);

				if ($tran_code_type == 1 || $tran_code_type == "") {
					$code = gen_max_code(1, $year_account, conv_date_db($_POST['date_receipt']));
					$tran_code = $code['code'];
					$tran_no = $code['max_id'];
				} else {
					$tran_code = $tran_code;
				...
				$tran_topic = ctext("บันทึกใบเสร็จรับเงินเลขที่" . $rec_code . "(" . $_POST['pay_fname'] . " " . $_POST['pay_lname'] . ")");

				$fields = array(
					"tran_code" => ctext($tran_code),
					"tran_topic" => $tran_topic,
					"tran_date" => ctext(conv_date_db($_POST['date_receipt'])),
					"post_status" => 1,
					"tran_type" => 1,
					...
					"create_by" => ($USER_BY),
					"reference_no" => ctext($rec_code),
					"reference_date" => conv_date_db($_POST['date_receipt']),
					"rec_id_all" => $rec_id
				);

				$tran_id = $db->db_insert($tablea_acc, $fields, 'y');

				unset($fields_up_rec);
				$fields_up_rec['tran_id'] = $tran_id;
				$db->db_update('F_RECEIPT', $fields_up_rec, " rec_id = '" . $rec_id . "' ");

				if ($_POST['call_type'] == 3 || $_POST['call_type'] == 2) {  // โอน
					$sql_link = "select acc_gl_id from ACC_BANK where bank_id ='" . $_POST['bank_id'] . "'";
					$query_link = $db->query($sql_link);
					$rec_link = $db->db_fetch_array($query_link);
				} else {  //1=สด  2=เคาเตอร์
					if ($_POST['call_type'] == 1) {
						$sql_link = "select acc_gl_id from acc_setup_link where setup_link_code='001' ";
						$query_link = $db->query($sql_link);
						$rec_link = $db->db_fetch_array($query_link);
					}
				}
				...
				$fields = array(
					...
					"acc_gl_id" => $rec_link["acc_gl_id"],
					"post_type" => 'D',
					"post_value" => $sum_total
				);
				...
				if (count($_POST['income_id']) > 0) {
					foreach ($_POST['income_id'] as $key => $val) {
						...
						if ($_POST['income_var'][$val] > 0) {
							...
							$fields = array(
								"tran_id" => ctext($tran_id),
								"acc_gl_id" => $arr_income_acc[$val],
								"post_type" => 'C',
								"post_value" => trim(str_replace($str_oder, "", $_POST['income_var'][$val]))
							);
...
?>
	...
	case "get_mem": {
		...
		?>
			<?php
			if ($_POST['col'] == '1') { ?>
				<div id="member_no_n_<?php echo $_POST['id_tb']; ?>" align="center"><?php echo disp_member_code($rec_mem['member_no']); ?></div>
			...
			<?php
				...
				<div id="full_name_n_<?php echo $_POST['id_tb']; ?>" align="left"><?php echo text($rec_mem['prefix_name'] . $rec_mem['fname'] . " " . $rec_mem['lname']); ?></div>
			<?php
			} else if ($_POST['col'] == '3') { ?>
				<div id="deposit_money_html_<?php echo $_POST['id_tb']; ?>" align="center"><?php echo number_format($rec_mem['deposit_money'], 2); ?></div>
	<?php
			}
		}
		break;
