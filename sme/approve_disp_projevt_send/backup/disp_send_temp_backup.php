...
///////// value  ////////////
$sql_val = "select * from prjp_report_product_temp where 1=1 and PRJP_ID = '".$PRJP_ID."' ";
...
//////////// desc  ///////////////
$sql_desc = "select * from product_desc_temp where 1=1 and PRJP_ID = '".$PRJP_ID."' ";
...
///////// value old  ////////////
$sql_valo = "select * from prjp_report_product_temp where 1=1 and PRJP_ID = '".$rec_head['PRJP_CON_ID']."' ";
...
///////// value old now ////////////
//echo $fbs;
$sql_val_now = "select * from prjp_report_product_temp where 1=1 and YEAR*100+MONTH< '".$fbs."' and PRJP_ID = '".$rec_head['PRJP_CON_ID']."'  ";
...
//////////// desc  ///////////////
$sql_desco = "select * from product_desc_temp where 1=1 and PRJP_ID = '".$rec_head['PRJP_CON_ID']."' ";
...
/////////query_file_include////////
$sql_val_include_result = "select * from prjp_report_result_temp where 1=1 and PRJP_ID = '".$PRJP_ID."' ";
...
//////////// desc  ///////////////
$sql_desc_include_result = "select * from result_desc_temp where 1=1 and PRJP_ID = '".$PRJP_ID."' ";
	...
	<div class="col-xs-12 col-sm-12 page-data-report">
		<div class="groupdata" >
			<form id="frm-search" method="post" action="<?php echo $_SERVER['PHP_SELF']; ?>" enctype="multipart/form-data">
				...
				<div class="row">  
						...
						<table width="100%" class="table table-bordered table-striped table-hover table-condensed">
						  <thead>
							<tr class="bgHead">
							  ...
							  <th width="1%" nowrap rowspan="2"><div align="center"><strong>ชื่อผลผลิต</strong></div></th>
							  ...
							  <th width="1%" ><div align="center"><strong>ผลที่ได้ของปี&nbsp;<?php echo $_SESSION['year_round']; ?></strong></div></th>
							  <th width="" nowrap><div align="center"><strong>รายละเอียดการดำเนินงาน/ปัญหาและอุปสรรค</strong></div></th>
							</tr>
							<tr class="bgHead">
							 <th width="1%" nowrap colspan="2"><div align="center"><strong><?php echo $month[$smho*1].$syho;?></strong></div></th>
							</tr>
							</thead>
							<tbody>
							<?php
							...
							<tr bgcolor="#FFFFFF">
							  <td align="center"><?php echo $i; ?>. 
								...
								<td align="left"><?php if($reco['TYPE_PRO_ID']!='9999'){ echo text($reco['TYPE_PRO_NAME']);}else{echo  "อื่น ๆ";}?> <?php echo "(".number_format($arr_pval_so[$reco['PRJP_PRODUCT_ID']], 2).")"; ?>
								</td>
								<td align="left"><?php echo number_format($reco['GOAL_VALUE'], 2);?></td>
								<td align="left"><?php if($reco['TYPE_PRO_ID']!='9999'){ echo text($reco['UNIT_NAME_TH']);}else{echo text($reco['UNIT_PRO_NAME']);}?>
					...
					<?php } ?>
					  ...
					  <table width="22%" class="table table-bordered table-striped table-hover table-condensed" id="tb_file_product_old">
						  <thead>
							<tr class="bgHead">
							  <th width="2%" rowspan="2"><div align="center"><strong>ลำดับ</strong></div></th>
							  <th width="20%" rowspan="2"><div align="center"><strong>ชื่อไฟล์</strong></div></th>
							  <th width="20%" rowspan="2"><div align="center"><strong>ไฟล์</strong></div></th>
								...
								<div class="row">  
									<div class="col-xs-12 col-sm-12">
										...
										foreach($m as $key=>$val){
											...
											$syh = substr($val,2,2);
										?>	
										<li id="tablist_<?php echo $val; ?>" class="re_act"><a onClick="tab_list(<?php echo $val; ?>)"><?php echo $month[$smh*1].$syh;?></a></li>
										<?php 
										}
										?>
								...
								<div class="col-xs-12 col-sm-12 tb_v" id="show_rb_<?php echo $val; ?>">
								  ...
								  <input name="YEAR_C[<?php echo $val;?>]" id="YEAR_C_<?php echo $val;?>" type="hidden" size="5" class="form-control number_format" value="<?php echo ($val); ?>">
									  ...
									  <thead>
										...
										<tr bgcolor="#FFFFFF">
										  ...
										  <input type="hidden" id="PRJP_PRODUCT_ID_DEL" name="PRJP_PRODUCT_ID_DEL[]" value="<?php echo $rec['PRJP_PRODUCT_ID']; ?>">
											...
											<td align="left"><?php if($rec['TYPE_PRO_ID']!='9999'){ echo text($rec['UNIT_NAME_TH']);}else{echo text($rec['UNIT_PRO_NAME']);}?>
										  ...
										  <td align="center">ผล</td>
											...
											<input name="PLAN_VALUE[<?php echo $val;?>][<?php echo $rec['PRJP_PRODUCT_ID']; ?>]" id="PLAN_VALUE_<?php echo $val;?>" type="text" size="5" class="form-control number_format" value="<?php echo number_format($arr_pval[$rec['PRJP_PRODUCT_ID']][$val], 2); ?>" onBlur="NumberFormat(this, 2);" style="text-align:right" <?php echo $distxt; ?>></td>
											 <td align="left">
											 รายละเอียดการดำเนินงาน : 
											 <br />
											 <textarea name="DESC_NAME[<?php echo $val;?>][<?php echo $rec['PRJP_PRODUCT_ID']; ?>]" <?php echo $distxt; ?> id="DESC_NAME_<?php echo $val;?>" cols="50" rows="3"><?php echo text($arr_desc[$rec['PRJP_PRODUCT_ID']][$val]); ?></textarea>
											 <br />
											  ปัญหาและอุปสรรค : 
											  <br />
											 <textarea  name="RICK_NAME[<?php echo $val;?>][<?php echo $rec['PRJP_PRODUCT_ID']; ?>]" <?php echo $distxt; ?> id="RICK_NAME_<?php echo $val;?>" cols="50" rows="3"><?php echo text($arr_rick[$rec['PRJP_PRODUCT_ID']][$val]); ?></textarea>
											 <br />
											 แนวทางแก้ไข :
											 <br />
											 <textarea  name="SOLUTION_NAME[<?php echo $val;?>][<?php echo $rec['PRJP_PRODUCT_ID']; ?>]" <?php echo $distxt; ?> id="SOLUTION_NAME_<?php echo $val;?>" cols="50" rows="3"><?php echo text($arr_solution[$rec['PRJP_PRODUCT_ID']][$val]); ?></textarea>
									...
									if($CHK_BDG_TYPE_ID != 4){
										include("disp_project_send_result_copy.php");
									}
									?>
								  ...
								  <div class="">
								  <!-- <div  style="margin-bottom:10px;"><a data-toggle="modal" class="btn btn-default" data-backdrop="static" href="javascript:void(0);" onClick="addrow_file();"><?php echo $img_save;?> เพิ่มเอกสาร</a><font color="red"> * กรุณาแนบไฟล์เอกสาร รายละเอียดที่เกี่ยวกับโครงการ สกุลไฟล์ .gif,jpg,Ms.Word,Ms.excel,PDF เป็นต้น ขนาดไฟล์ต่อ 1 ไฟล์ ห้ามเกิน 10MB</font></div> -->
								  <table width="22%" class="table table-bordered table-striped table-hover table-condensed" id="tb_file_product">
									  <thead>
										...
										<?php 
											$i=0;
											$sql_file = "select * from prjp_product_file_temp where PRJP_ID = '".$PRJP_ID."' order by PRODUCT_FILE_ID ASC";
											$query_file = $db->query($sql_file);
											while($rec_file = $db->db_fetch_array($query_file)){
												$i++;
										?>
										<tr>
											...
											<a class="btn btn-danger" data-toggle="modal" class="btn btn-default btn-xs" data-backdrop="static"  onClick="del_row(this);"><?php echo $img_del;?> ลบ</a>
											</td>
										</tr>	
										<?php 
											}
				...
				<?php include("disp_project_send_act_task_copy_temp.php");
