				...
				while($rec_result_copy = $db->db_fetch_array($query_result_copy)){
					...
					<tr bgcolor="#FFFFFF">
					  ...
					  <input type="hidden" id="PRJP_RESULT_ID_DEL" name="PRJP_RESULT_ID_DEL[]" value="<?php echo $rec_result_copy['PRJP_RESULT_ID']; ?>">
						...
						<input name="PLAN_VALUE_INCLUDE[<?php echo $val;?>][<?php echo $rec_result_copy['PRJP_RESULT_ID']; ?>]" id="PLAN_VALUE_INCLUDE_<?php echo $val;?>" type="text" size="5" class="form-control number_format" value="<?php echo number_format($arr_pval_include_result[$rec_result_copy['PRJP_RESULT_ID']][$val],2); ?>" onBlur="NumberFormat(this,2);" style="text-align:right" <?php echo $disables_txt; ?>></td>
						 <td align="left">
						 รายละเอียดการดำเนินงาน :<br>
						 <textarea name="DESC_NAME_INCLUDE[<?php echo $val;?>][<?php echo $rec_result_copy['PRJP_RESULT_ID']; ?>]" <?php echo $readonly_txt; ?> id="DESC_NAME_INCLUDE_<?php echo $val;?>" cols="50" rows="3"><?php echo text($arr_desc_include_restule[$rec_result_copy['PRJP_RESULT_ID']][$val]); ?></textarea><br>
						 ปัญหาและอุปสรรค :<br>
						 <textarea name="RICK_NAME_INCLUDE[<?php echo $val;?>][<?php echo $rec_result_copy['PRJP_RESULT_ID']; ?>]" <?php echo $readonly_txt; ?> id="RICK_NAME_INCLUDE_<?php echo $val;?>" cols="50" rows="3"><?php echo text($arr_rick_include_result[$rec_result_copy['PRJP_RESULT_ID']][$val]); ?></textarea><br />
						 แนวทางแก้ไข :<br>
						 <textarea name="SOLUTION_NAME_INCLUDE[<?php echo $val;?>][<?php echo $rec_result_copy['PRJP_RESULT_ID']; ?>]" <?php echo $readonly_txt; ?> id="SOLUTION_NAME_INCLUDE_<?php echo $val;?>" cols="50" rows="3"><?php echo text($arr_solution_include_result[$rec_result_copy['PRJP_RESULT_ID']][$val]); ?></textarea>
						 </td>
					</tr>
					<?php 
						$i_result_copy++;
