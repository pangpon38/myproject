...
switch ($_GET['proc']) {
  case 'getLogin':
    ...
    if ($request['username'] != "" & $request['password'] != "") {
      ...
      $sqlChk_c = "SELECT COUNT(USER_ID) AS NUM FROM M_USER U
       WHERE CONVERT(VARCHAR(32), HashBytes('MD5', CONCAT(U.ID_CARD_NO,'z4HQRQ%-Y=1DXzw')), 2) = '" . $request['username'] . "' AND CONVERT(VARCHAR(32), HashBytes('MD5', CONCAT(U.PASSWORD,'z4HQRQ%-Y=1DXzw')), 2) = '" . $password . "' ";
      ...
      if ($numrow['NUM'] > 0) {
        ...
        $data = array('status_login' => 'Y', 'msg' => 'สำเร็จ', 'detail_member' => $data_login);
        echo json_encode($data);
      } else {
        $data = array('status_login' => 'N', 'msg' => 'กรุณากรอกข้อมูล USERNAME และ PASSWORD ให้ถูกต้อง');
        echo json_encode($data);
      }
    } else {
      $data = array('status_login' => 'N', 'msg' => 'กรุณากรอกข้อมูล USERNAME และ PASSWORD');
      echo json_encode($data);
    }
    break;
  case 'generateToken':
  ...
  case 'getMember':
    ...
    $sql = "select 	M_MEMBER.member_no
        ...
        LEFT JOIN M_TYPE on M_TYPE.m_cate_id = M_MEMBER.m_cate_id
        LEFT JOIN PREFIX on M_MEMBER.prefix_id = PREFIX.prefix_id
        where CONVERT(VARCHAR(32), HashBytes('MD5', CONCAT(M_MEMBER.member_no,'z4HQRQ%-Y=1DXzw')), 2) = '" . $request['member_no']  . "'
        ";
    $aaa = $db->query($sql);
  ...
  case 'getAddressSendDoc':
    $baacChapa->checkAuth($request);
    $sql_dep = "SELECT mb.CHAPA_CLOUD_ID , mb.DEP_NAME,ud.MEMBER_NO FROM FRM_USER_DEP ud LEFT JOIN  M_BASIC mb ON ud.DEP_ID = mb.BASIC_ID  WHERE CONVERT(VARCHAR(32), HashBytes('MD5', CONCAT(ud.WFR_ID,'z4HQRQ%-Y=1DXzw')), 2) = '" . $request['user_id'] . "' ";
    $q_dep = $db->query($sql_dep);
    $data_dep  = array();
    while ($row_dep = $db->db_fetch_array($q_dep)) {
      $EWT_DB_NAME = "baac_chapa_" . $row_dep['CHAPA_CLOUD_ID'];
  ...
  case 'getReportPayment':
    ...
    $sql_chk_memid = "select member_id from M_MEMBER where CONVERT(VARCHAR(32), HashBytes('MD5', CONCAT(member_no,'z4HQRQ%-Y=1DXzw')), 2) = '" . $request['member_no']  . "' ";
    ...
    $sql_payment = "select
        ...
        where
        M_MEMBER.member_id = '" . $row_chk_memid['member_id'] . "' $txt ORDER BY F_RECEIPT.date_receipt DESC";
    ...
    while ($row_payment = $db->db_fetch_array($q_payment)) {
      ...
      array_push($report_payment, array('rec_code' => $row_payment['rec_code'], 'date_receipt' =>  chgDate(date_format($row_payment['date_receipt'], 'Y-m-d')), 'sum_total' => number_format($row_payment['sum_total'], 2), 'call_type' => $type_payment, 'status_use' => $row_payment['status_use']));
      $i++;
    }
  ...
  case 'getListPayment':
    ...
    $sql_date_payment = "select
        ...
        group by
        F_RECEIPT.date_receipt, F_RECEIPT.rec_id, F_RECEIPT_DE.member_id,F_RECEIPT.code_receipt,F_RECEIPT.status_use
        order by
    ...
    while ($row_date_payment = $db->db_fetch_array($q_date_payment)) {
      ...
      while ($row_list = $db->db_fetch_array($q_list)) {
        array_push($list_payment[$row_date_payment['rec_id']], array('income_name_th' => $row_list['income_name_th'], 'money' => number_format($row_list['money'], 2)));
        $i++;
      }
      array_push($list,  array('code_receipt' => $row_date_payment['code_receipt'], 'date' => chgDate(date_format($row_date_payment['date_receipt'], 'Y-m-d')), 'listname' => $list_payment[$row_date_payment['rec_id']], 'status_use' => $row_date_payment['status_use']));
    }
  ...
  case 'insAssociation':
    ...
    /*$sql_chk_memid = "select USER_ID from M_USER where MEMBER_NO = '" . $request['member_no']  . "' ";
        $q_chk_memid = $db->query($sql_chk_memid);
        $row_chk_memid = $db->db_fetch_array($q_chk_memid);*/
    $sql = "SELECT F_ID FROM FRM_USER_DEP WHERE CONVERT(VARCHAR(32), HashBytes('MD5', CONCAT(WFR_ID,'z4HQRQ%-Y=1DXzw')), 2) = '" . $request['user_id'] . "' AND CONVERT(VARCHAR(32), HashBytes('MD5', CONCAT(MEMBER_NO,'z4HQRQ%-Y=1DXzw')), 2) = '" . $request['member_no'] . "' AND DEP_ID = '" . $request['dep_id'] . "' ";
    $q = $db->query($sql);
    $row = $db->db_fetch_array($q);

    $sql_d = $db->query("SELECT CHAPA_CLOUD_ID FROM M_BASIC WHERE CONVERT(VARCHAR(32), HashBytes('MD5', CONCAT(M_MEMBER.member_no,'z4HQRQ%-Y=1DXzw')), 2) = '" . $request['dep_id'] . "' ");
    ...
    if (!empty($rec_m['member_id'])) {
      ...
      if (empty($row['F_ID'])) {
        ...
        $data = array('status' => 'N', 'msg' => 'มีข้อมูลสมาคมนี้แล้ว');
        echo json_encode($data);
      }
    } else {
      $data = array('status' => 'N', 'msg' => 'ยังไม่มีข้อมูลสมาชิกในสมาคมนี้');
      echo json_encode($data);
    }
  ...
  case 'getBank':
    ...
    $data_bank = array('ธนาคารเพื่อการเกษตรและสหกรณ์การเกษตร', 'ธนาคารไทยพาณิชย์ จำกัด (มหาชน)', 'ธนาคารกสิกรไทย จำกัด (มหาชน)', 'ธนาคารกรุงศรีอยุธยา จำกัด (มหาชน)', 'ธนาคารออมสิน', 'ธนาคารกรุงเทพ จำกัด (มหาชน)', 'ธนาคารกรุงไทย จำกัด (มหาชน)', 'ธนาคารทหารไทย จำกัด (มหาชน)', 'ธนาคารยูโอบี จำกัด (มหาชน)', 'ธนาคารธนชาต จำกัด (มหาชน)');
    $c_bank = count($data_bank);
    $data = array();
    for ($i = 0; $i < $c_bank; $i++) {
      array_push($data, array('bankname' => $data_bank[$i]));
    }
    $data_c = array('deposit_money' => number_format($pay_total, 2), 'datachapa' => $data);
  ...
  case 'changePass':
    ...
    if (!empty($q['USER_ID'])) {
      ...
      $data_1 = array('status' => 'Y', 'msg' => 'สำเร็จ');
    } else {
      $data_1 = array('status' => 'N', 'msg' => 'username หรือ password ไม่ถูกต้อง');
    }
