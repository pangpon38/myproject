	...
	<div class="col-xs-12 col-sm-12">
        <ol class="breadcrumb">
         ...
         <li><a href="disp_send_project.php?<?php echo url2code("menu_id=".$menu_id."&menu_sub_id=".$menu_sub_id);?>"><?php echo Showmenu($menu_sub_id);?></a></li>
          <li class="active">แนวทางในการบริหารความเสี่ยง</li>
        </ol>
    </div>
								...
								for($row=1;$row<=$row_wh;$row++){ ?>
										...
										<table width="22%" class="table table-bordered table-striped table-hover table-condensed">
											<thead>
												<tr class="bgHead table-head-money">
													...
													<th width="" ><div align="center"><strong>ความเสี่ยง</strong></div></th>
													<th width="" ><div align="center"><strong>เกรดที่ได้รับ</strong></div></th>
													<th width="" ><div align="center"><strong>แนวทางในการบริหารความเสี่ยง</strong></div></th>
													<th width="" ><div align="center"><strong>ผู้ที่รับผิดชอบต่อการนำแนวทางไปใช้</strong></div></th>
													<th width="" ><div align="center"><strong>ต้นทุนที่จะเกิดขึ้น จากการนำแนวทางมาใช้</strong></div></th>
													<th width="" ><div align="center"><strong>ข้อมูลการวิเคราะห์ความเสี่ยงของงาน</strong></div></th>
												</tr>
											</thead>
											<tbody>
												<?php
												if($num_rows > 0){
													...
													while($rec = $db->db_fetch_array($query)){
														...
														<tr bgcolor="#FFFFFF" class="">
															...
															<td align="left">
																<textarea rows="3" style="width:100%; min-width:100px;" class="prjp-name-show" disabled=""><?php echo text($rec['risk_name']);?></textarea>
															</td>
															<td align="left" style="width:120px !important;">
																<textarea rows="3" style="width:100%; min-width:100px;" class="prjp-name-show" disabled=""><?php echo text($rec['risk_grade']);?></textarea>
															</td>
															<td align="left">
																<textarea rows="3" style="width:100%; min-width:100px;" class="prjp-name-show" disabled=""><?php echo text($rec['risk_manage']);?></textarea>
															</td>
															<td align="left">
																<textarea rows="3" style="width:100%; min-width:100px;" class="prjp-name-show" disabled=""><?php echo text($rec['risk_response_by']);?></textarea>
															</td>
															<td align="left">
																<textarea rows="3" style="width:100%; min-width:100px;" class="prjp-name-show" disabled=""><?php echo text($rec['risk_cost']);?></textarea>
															</td>
															<td align="left">
																<textarea rows="3" style="width:100%;" name="report_detail[<?php echo $rec['project_id'] ?>]" placeholder="ข้อมูลการวิเคราะห์ความเสี่ยงของงาน"><?php echo text($rec['report_detail']);?></textarea>
															</td>
														</tr>
														<?php
														$l++;
									...
									<div class="clearfix" align="center">
										<?php if($_SESSION['sys_status_edit']=='1' && ($_SESSION["sys_group_id"] == '5' || $distxt != 'readonly')){ ?>
											<div class="row"><button type="button" class="btn btn-success" onClick=" if(confirm('ยืนยันการบันทึก ?')){ $.post('disp_project_task_report_risk.php?proc=save_risk', $('#frm-search').serialize(), function(rst){ alert(rst); }) } ">บันทึก</button></div>
										<?php } ?>
									</div>
