...
switch ($_GET['proc']) {
    case 'getLogin':
        ...
        if (empty($request['username'])) {
            $data = ['status_login' => 'N', 'msg' => 'กรุณากรอกข้อมูล USERNAME และ PASSWORD'];
            echo json_encode($data);
            exit;
        }

        if (!is_numeric($request['username'])) {
            $data = ['status_login' => 'N', 'msg' => 'กรุณากรอกข้อมูล USERNAME และ PASSWORD ให้ถูกต้อง'];
            echo json_encode($data);
            exit;
        }
        ...
        if ($numrow['NUM'] > 0) {
            ...
            $data = ['status_login' => 'Y', 'msg' => 'สำเร็จ', 'detail_member' => $data_login];
            echo json_encode($data);
    ...
    case 'generateToken':
        ...
        $response = $baacChapa->generateToken(['APP_CODE' => 'baacChapa', 'APP_NAME' => 'baacChapa']);
    ...
    case 'getMember':
        ...
        if (!is_numeric($request['member_no'])) {
            http_response_code(404);
            exit;
        }
        ...
        $sql = "SELECT
                ...
                FROM
                    ...
                    LEFT JOIN PREFIX ON M_MEMBER.prefix_id = PREFIX.prefix_id
                WHERE
                    M_MEMBER.member_no = '" . $request['member_no'] . "'";
        $exc = $db->query($sql);
        ...
        echo json_encode(['datachapa' => $member]);
    ...
    case 'getMemberDetail':
        ...
        echo json_encode(['datachapa' => $member_detail]);
    ...
    case 'getAddressSendDoc':
        ...
        echo json_encode(['datachapa' => $data_dep]);
    ...
    case 'changeAddressSendDoc':
        ...
        if (empty($request['home_no'])) {
            $data = ['status_insert' => 'N', 'msg' => 'ข้อมูลผิดพลาด'];
            echo json_encode($data);
            exit;
        }

        if (empty($request['prov_code'])) {
            $data = ['status_insert' => 'N', 'msg' => 'ข้อมูลผิดพลาด1'];
            echo json_encode($data);
            exit;
        }
        ...
        echo json_encode(['datachapa' => ['status_insert' => $status, 'msg' => $msg]]);
    ...
    case 'getReportPayment':
        ...
        if (empty($request['year'])) {
            $data = ['status' => 'N', 'msg' => 'ข้อมูลผิดพลาด'];
            echo json_encode($data);
            exit;
        }
        ...
        echo json_encode(['reportpayment' => $report_payment]);
    ...
    case 'getListPayment':
        ...
        echo json_encode(['deposit_money' => $row_de['deposit_money'], 'datachapa' => $list]);
    ...
    case 'insAssociation':
        ...
        $sql = "SELECT
                ...
                WHERE
                    ...
                    MEMBER_NO = '" . $request['member_no'] . "' AND
                    DEP_ID = '" . $request['dep_id'] . "' ";
        $q = $db->query($sql);
        $row = $db->db_fetch_array($q);
        if ($row['F_ID']) {
            $data = ['status' => 'N', 'msg' => 'มีข้อมูลสมาคมนี้แล้ว'];
            echo json_encode($data);
            exit;
        }
        ...
        if (empty($rec_m['member_id'])) {
            $data = ['status' => 'N', 'msg' => 'ยังไม่มีข้อมูลสมาชิกในสมาคมนี้'];
            echo json_encode($data);
            exit;
        }
    ...
    case 'forGetpass':
        ...
        echo json_encode(['status' => 'Y', 'link' => 'https://www.smartchapa.com/webapp/forget_password.php']);
    ...
    case 'getBank':
        ...
        $data_c = ['deposit_money' => number_format($pay_total, 2), 'datachapa' => $data];
    ...
    case 'getProvince':
        ...
        echo json_encode(['datachapa' => $data_province]);
    ...
    case 'getAmphur':
        ...
        echo json_encode(['datachapa' => $data_amphur]);
    ...
    case 'getTambon':
        ...
        echo json_encode(['datachapa' => $data_tambon]);
    ...
    case 'changePass':
        ...
        echo json_encode(['datachapa' => $data]);
