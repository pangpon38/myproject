...
SET @e_date = convert(VARCHAR(10), CONCAT(YEAR(Getdate()),'-12-01'), 23)
...
SET @SQLString = 'SELECT @c_member_2=ISNULL(COUNT(DISTINCT M_MEMBER_OUT.member_id),0)
 ...
 AND M_MEMBER_OUT.request_date <='''+@e_date+''' and F_PAYMENT_DE.cost_id in (''22'',''33'' )'
...
SET @SQLString = 'select @c_member_3=ISNULL(COUNT(DISTINCT M_ARREAR.member_id),0) from '+@DBNAME+'.dbo.M_ARREAR 
where M_ARREAR.member_out_id in ( select M_MEMBER_OUT.member_out_id from '+@DBNAME+'.dbo.M_MEMBER_OUT where 
M_MEMBER_OUT.request_date >='''+@s_date+''' AND M_MEMBER_OUT.request_date <='''+@e_date+''' ) 
...
SET @SQLString = 'select @s_money_3=ISNULL(SUM(M_ARREAR.ARREAR_MONEY),0) from '+@DBNAME+'.dbo.M_ARREAR 
where M_ARREAR.member_out_id in ( select M_MEMBER_OUT.member_out_id from '+@DBNAME+'.dbo.M_MEMBER_OUT where 
M_MEMBER_OUT.request_date >='''+@s_date+''' AND M_MEMBER_OUT.request_date <='''+@e_date+''' ) 
and M_ARREAR.STATUS_PAID_DIED = 0 and M_ARREAR.FLAG_PAID = 1'
...
--- ลบข้อมูลออกจาก quater ก่อนหน้า
	SET @SQLString ='DELETE FROM M_REPORT_MONEY WHERE quater = '''+convert(VARCHAR(10),@quater)+''' 
	AND DATE_RUN <> '''+convert(VARCHAR(10),@date_run)+''' AND year_money =  '''+convert(VARCHAR(10),@S_YEAR)+''' '
